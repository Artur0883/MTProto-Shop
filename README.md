...
TEST_AUTO_ISSUE_ACCESS=true
```

Все реальные секреты должны быть только в `.env`.

`TEST_AUTO_ISSUE_ACCESS=true` включает тестовый режим: клиент выбирает тариф в боте и сразу получает личную proxy-ссылку. Для боевой продажи без автоматической выдачи поставьте:

```env
TEST_AUTO_ISSUE_ACCESS=false
```
...
Кнопка «Купить доступ» показывает inline-кнопки тарифов:
...
В тестовом режиме `TEST_AUTO_ISSUE_ACCESS=true` бот сразу создаёт или продлевает подписку и отправляет личную proxy-ссылку. Если `TEST_AUTO_ISSUE_ACCESS=false`, бот не выдаёт ключ автоматически и просит написать администратору для оплаты.

После выдачи или отключения доступа proxy всё ещё должен перечитать config:
...
5. Выбирает тариф 1/3/6/12 месяцев.
...
Чтобы кнопка работала, один раз установите watcher на VPS: откройте `mtp` и выберите пункт `19` — **🛡 Установить watcher автоматического рестарта (одноразово)**.
